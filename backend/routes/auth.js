  ...
  try {
    ...
    if (finalRole === 'student') {
      if (!email.toLowerCase().endsWith('@klu.ac.in')) {
        return res.status(400).json({ error: 'Students must register with a @klu.ac.in email address.' });
      }
    }
    ...
    await db.run(
      `INSERT INTO users (id, name, email, password_hash, role, department, year, register_number, batch, section, faculty_advisor)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        name,
  ...
  try {
    ...
    const user = await db.get(
      `SELECT id, name, email, role, department, year,
              register_number, batch, section, faculty_advisor, created_at
       FROM users WHERE id = ?`,
      [req.user.id]
    );
